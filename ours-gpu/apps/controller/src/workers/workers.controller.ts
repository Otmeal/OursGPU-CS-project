import { Body, Controller, Get, NotFoundException, Param, Post, Logger } from '@nestjs/common';
...
export class WorkersController {
    ...
    promise
      .then((workers) => {
        ...
        if (workers.length) {
          logger.debug(
            `Workers: ${workers
              .map((w) => `${w.id}@${w.orgId} running=${w.running} conc=${w.concurrency}`)
              .join(', ')}`,
          );
        }
      })
      .catch((err) => {
        const logger = new Logger(WorkersController.name);
        logger.warn(`Failed to list workers: ${err?.message ?? err}`);
      });
    return promise;
  }
  ...
  @Post(':workerId/jobs')
  async dispatch(@Param('workerId') workerId: string, @Body() body: CreateJobDto & { workerId?: string }) {
    ...
    const { workerId: _ignored, ...jobDto } = body as any;
    const job = await this.jobs.create(jobDto);
