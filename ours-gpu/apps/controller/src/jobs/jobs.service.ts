...
export class JobsService {
  ...
  create(dto: CreateJobDto): Promise<Job> {
    return this.prisma.job.create({ data: { ...dto, status: JobStatus.REQUESTED } });
  }
  ...
  nextSchedulable(): Promise<Job | null> {
    return this.prisma.job.findFirst({ where: { status: JobStatus.REQUESTED }, orderBy: [{ priority: 'desc' }, { createdAt: 'asc' }] });
  }

  markScheduled(id: string, workerId: string) {
    return this.prisma.job.update({ where: { id }, data: { status: JobStatus.SCHEDULED, assignedWorkerId: workerId } });
  }

  markResult(id: string, success: boolean) {
    return this.prisma.job.update({ where: { id }, data: { status: success ? JobStatus.DONE : JobStatus.FAILED } });
  }
}
